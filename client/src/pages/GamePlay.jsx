import { ArrowLeft, Coins, RotateCcw, Timer, Trophy, X } from 'lucide-react';
...
function TapGame({ onScore }) {
  ...
  useEffect(() => { const t = setInterval(() => setTarget({ x: 12 + Math.random() * 76, y: 18 + Math.random() * 64 }), 700); return () => clearInterval(t); }, []);
  return <div className="play-area tap-area"><div className="game-instruction">Tap the target as many times as you can.</div><button className="target" style={{ left: `${target.x}%`, top: `${target.y}%` }} onClick={() => onScore(10)}>+10</button></div>;
}
...
function MemoryGame({ onScore }) {
  ...
  return <div className="play-area memory-area"><div className="game-instruction">Memorize the highlighted tiles. Round {round}.</div><div className="memory-grid">{pattern.map((active, i) => <button key={i} className={(showPattern && active) || selected.includes(i) ? 'memory-tile active' : 'memory-tile'} onClick={() => tap(i)} />)}</div></div>;
}
...
export default function GamePlay() {
  ...
  const finish = async () => {
    ...
    try { const data = await api.completeGame(sessionId, score); setResult(data); setEnded(true); await refreshWallet(); }
    catch (error) { setResult({ error: error.message }); setEnded(true); }
  };

  useEffect(() => { timer.current = setInterval(() => setTime(t => t <= 1 ? 0 : t - 1), 1000); return () => clearInterval(timer.current); }, []);
  useEffect(() => { if (time === 0 && !ended) finish(); }, [time, ended]);
  ...
  const revive = async () => { try { await api.reviveGame(sessionId); completedRef.current = false; setRevived(true); setEnded(false); setTime(15); setResult(null); timer.current = setInterval(() => setTime(t => t <= 1 ? 0 : t - 1), 1000); } catch (error) { setResult({ error: error.message }); } };
  const reward = result?.earnedCoins ?? 0;
